...
const Component: FC<Props> = () => {
  const {
    users, existUsers, setExistUsers, existTaskTypes, setExistTaskTypes, taskTypes,
          ...
          {users.map((user) => (
            <label
              ...
              className={classNames(
                styles.user,
                !existUsers.find((us) => us === user.id) && styles.userDisabled,
              )}
            >
              <input
              ...
              <Avatar
                ...
                image={user.avatar}
                alt={[user.firstName[0], user.lastName[0]].join('')}
              />
        ...
        <Spoiler title='Departments'>
          {taskTypes.map((type) => (
            <label
              ...
              className={classNames(
                styles.user,
                !existTaskTypes.find((tp) => tp === type.id) && styles.userDisabled,
              )}
            >
              <input
                className={styles.userCheckbox}
                ...
                onChange={() => {
                  setExistTaskTypes(
                    existTaskTypes.find((tp) => tp === type.id)
