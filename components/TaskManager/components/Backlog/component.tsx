  ...
  return (
    <div className={styles.backdropButtonWrapper}>
      <div
        ...
        className={classNames(styles.backdropButton, isOpen && styles.backdropButtonVariant.open)}
      >
        <DroppableZone
          ...
          render={({ inFocus, ...props }) => (
            <div className={styles.backlogDropZone} {...props} />
          )}
        />
        {!isOpen && backlogTasks.map(task => (
          <ElementPosition key={task.id} order={task.id} id={task.id} className={styles.backlogTask} />
        ))}
        <svg className={styles.backdropButtonIcon} fill='currentColor' aria-hidden="true" viewBox="0 0 24 24">
        ...
        {isOpen && (
          <div className={styles.backlogTaskList}>
            {backlogTasks.map(task => (
              <ElementPosition key={task.id} order={task.id} id={task.id} className={styles.backlogTaskItem} />
            ))}
          </div>
        )}
      </div>
